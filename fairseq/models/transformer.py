...
class TransformerModel(FairseqEncoderDecoderModel):
    ...
    def add_args(parser):
        ...
        # fmt: off
        parser.add_argument('--src-drop', type=float, metavar='D', default=0.0,
                            help='dropout probability')
        parser.add_argument('--tgt-drop', type=float, metavar='D', default=0.0,
                            help='dropout probability')
    ...
    def forward(
        ...
        RTD_loss = 0
        DTP_loss = 0
        if self.training and self.args.RTD:
        ...
        if self.training and self.args.DTP:
            ...
            DTP_loss = -logits.gather(dim=-1, index=src_tokens.t().unsqueeze(-1)).squeeze(-1).mul(self.encoder.drop_targets.eq(1)).sum()
        decoder_out[1]['aux_loss'] = DTP_loss + RTD_loss
        return decoder_out
...
class TransformerEncoder(FairseqEncoder):
    ...
    def forward(
        ...
        if self.drop_method != 'none' and self.training:
            DROP_MATRIX = torch.empty(src_tokens.shape, device='cuda').fill_(self.src_drop).bernoulli().long()
        ...
        if self.drop_method != 'none' and not self.training:
           x = x * (1 - self.src_drop)
        # B x T x C -> T x B x C
        x = x.transpose(0, 1)
...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def extract_features(
        ...
        """
        if self.drop_method != 'none' and self.training:
            DROP_MATRIX = torch.empty(prev_output_tokens.shape, device='cuda').fill_(self.tgt_drop).bernoulli().long()
            pad = prev_output_tokens.eq(1) # padding index
            DROP_MATRIX[pad] = 0
            prev_output_tokens = prev_output_tokens * (1 - DROP_MATRIX) + self.mask * DROP_MATRIX # replacement
